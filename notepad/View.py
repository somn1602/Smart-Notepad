...
class Notepad:
    ...
    def set_status_new_1(self):
        self.present_date=date.today()
        self.status_bar1 = ttk.Label(self.root, text='Time_taken')
        self.status_bar1.pack(side=tk.BOTTOM)
        one_thread = Thread(target=self.timer_start)
        one_thread.start()
    ...
    def exit_func(self, event=None):
        ...
        try:
            ...
            messagebox.showerror("App Error","Error is closing")
            print(traceback.format_exc())
        finally:
            messagebox.showerror("Thank You","Have a good day")
    ...
    def open_secure_file(self):
        ...
        if len(self.sel_file_index_tuple) != 0:
            ...
            if self.db_controller.is_secure_file(base):
                pwd = self.get_file_pwd()
    ...
    def get_file_pwd(self):
        access_pwd = simpledialog.askstring("Password", "Please Enter Acceess Password")
        return access_pwd
    ...
    def timer_start(self):
        ...
        while True:
            ...
            if minute>59:
                ...
                minute=0
            second=second+1
            self.status_bar1.config(text=f'Date - {self.present_date}     Time Taken - {hour}:{minute}:{second}')
            time.sleep(1)
